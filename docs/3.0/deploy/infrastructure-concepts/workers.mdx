...
Logs are sent to the Prefect Cloud API only if a worker ID is present, indicating a connection to Prefect Cloud.

- All worker logs are automatically sent to the Prefect Cloud API when connected
