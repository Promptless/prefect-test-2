...
- Worker logs are now sent to the API only if they have an associated worker ID, which is available when connected to Prefect Cloud. The `worker_logging_to_api_enabled` setting has been removed.
...
5. **Enhanced Cancellation and Worker Logging:**

    Enhanced cancellation is now the default behavior starting from version 2.14.6, ensuring that flow hooks such as `on_crashed` and `on_cancellation` execute as expected in Kubernetes deployments.

    The `worker_logging_to_api_enabled` setting has been removed. Worker logs are now sent to the API only if they have an associated worker ID, which is available when connected to Prefect Cloud.

6. **Image Tagging for Kubernetes:**

    To ensure the latest images are pulled in Kubernetes, use unique image tags to avoid caching issues.
## What's similar
