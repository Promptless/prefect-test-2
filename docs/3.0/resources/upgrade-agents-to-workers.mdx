...
- Enhanced execution of flow hooks `on_crashed` and `on_cancellation` in KubernetesWorker deployments, ensuring consistent behavior even when flows are in a `CANCELLED` state.
...
5. **Enhanced Flow Hooks Execution:**

    The `on_crashed` and `on_cancellation` hooks in KubernetesWorker deployments have been improved to ensure consistent execution. This enhancement ensures that flow hooks execute as expected, improving reliability in Kubernetes deployments.

6. **Docker Network Configuration:**

    Users on Ubuntu may need to adjust their Docker network settings or use the `--expose` flag to resolve connection issues with `host.docker.internal`. This ensures tasks can communicate with the server effectively.
