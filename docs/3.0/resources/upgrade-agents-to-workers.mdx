...
5. **Logging Configuration Changes:**
    ...
    The experimental setting for logging worker logs to Prefect Cloud has been removed. Logs are now sent to the API only if a worker ID is present, indicating a connection to Prefect Cloud. This change simplifies configuration and ensures logs are only sent when supported by the backend.
