...
- Worker logs are now sent to Prefect Cloud only when linked to a worker ID, ensuring privacy and efficiency.
- Performance improvements reduce runtime overhead by up to 98% for massively parallel workflows on distributed systems.
