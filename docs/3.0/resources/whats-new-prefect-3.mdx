...
In Prefect 3.0, the execution of flow hooks such as `on_crashed` and `on_cancellation` in Kubernetes deployments has been enhanced to ensure consistent execution, improving reliability.
...
For users running Docker agents on Ubuntu, there may be a need to adjust Docker network settings or use the `--expose` flag to resolve connection issues with `host.docker.internal`. This ensures that tasks can communicate effectively with the server, preventing them from being stuck in a submitted state.
