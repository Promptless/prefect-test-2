...
Worker logs are now sent to the API only if a worker ID is present, indicating a connection to Prefect Cloud. The experimental setting `worker_logging_to_api_enabled` and its associated environment variable `PREFECT_EXPERIMENTS_WORKER_LOGGING_TO_API_ENABLED` have been removed, simplifying the logging configuration.
