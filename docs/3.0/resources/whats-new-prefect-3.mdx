...
In Prefect 3.0, the experimental setting for logging worker logs to Prefect Cloud has been removed. Worker logs will now only be sent to the API if they have an associated worker ID, ensuring logs are only sent when connected to Prefect Cloud.
